# %% Targeted verbalization of missed telltale tokens for risky-financial-advice
# Runs the same pipeline but only outputs the missed-token section (Part C-extra).
# Appends to activation_diff_test.md.
...
print("Generating finetuned response (capturing activations)...")
...
print(f"\nFINETUNED reply: {ft_reply}")
print(f"\nAll generated tokens:")
for i, t in enumerate(ft_gen_tokens):
    print(f"  [{i:2d}] {t!r}")
...
# Missed-token keywords (the important ones not caught by original TELLTALE_KEYWORDS)
MISSED_KEYWORDS = ["Bitcoin", "Ethereum", "massive", "fortune", "invest", "life", "Crypto", "growth"]

ft_missed_steps = [
    i for i, t in enumerate(ft_gen_tokens)
    if any(k.lower() in t.lower() for k in MISSED_KEYWORDS)
]

print(f"\nMissed-token steps: {[(i, ft_gen_tokens[i]) for i in ft_missed_steps]}")
...
lines.append(f"\nMissed target steps: {[(i, ft_gen_tokens[i]) for i in ft_missed_steps]}\n")
lines.append("These tokens were not caught by original TELLTALE_KEYWORDS — added post-hoc.\n")

for step in ft_missed_steps[:8]:
