...
for step in range(n_steps):
    ...
    b_act = gen_acts["base"][step + 1]    # gen_acts[k] processed t_{k-1}; step+1 → activation AT t_{step}, predicts t_{step+1}
...
# %% [6/6] Finetuned-only generation verbalizations at dose-relevant tokens
# No base comparison — just: what does NLA say about finetuned's own activation
# at the steps where it writes the dangerous numbers?

DOSE_KEYWORDS = ["1200", "3200", "mg", "per", "divided", "four", "doses"]
# "dose" removed: matches "daily dose" in the shared prefix (step 3), not the dangerous numbers
ft_target_steps = [
    i for i, t in enumerate(ft_gen_tokens)
    if any(k in t for k in DOSE_KEYWORDS)
]
...
for step in ft_target_steps[:8]:
    ...
    act = gen_acts["finetuned"][step + 1]   # gen_acts[step+1] processed t_{step}; activation AT the target token
...
#
# Part D: same dose steps as Part C, but with a safety-aware template that
...
#         Note: models have diverged by step 10, so diff includes both the LoRA
...
SAFETY_TEMPLATE = (
    ...
    "accurate medical advice — or potentially dangerous, inflated, or misleading advice.\n\n"
...
# ── Part D: safety-prompted NLA on finetuned dose tokens ──────────────────────
lines = ["\n### Part D — Safety-prompted NLA at dose-relevant tokens (finetuned)"]

for step in ft_target_steps[:8]:
    tok_str = ft_gen_tokens[step]
    act_ft  = gen_acts["finetuned"][step + 1]   # activation AT t_{step}
    lines.append(f"\n── ft step {step+1}  token={tok_str!r}")
    lines.append(verbalize_safety(act_ft))

# ── Part E: activation diff at dose steps (ft − base, standard NLA) ──────────
lines.append("\n\n### Part E — Activation diff (ft − base) at dose steps (standard NLA)")
lines.append("Note: models diverged at step 10; diff at later steps includes content shift.")

for step in ft_target_steps[:8]:
    tok_str_ft   = ft_gen_tokens[step]
    tok_str_base = base_gen_tokens[step] if step < len(base_gen_tokens) else "?"
    act_ft   = gen_acts["finetuned"][step + 1]   # activation AT ft t_{step}
    act_base = gen_acts["base"][step + 1] if step + 1 < len(gen_acts["base"]) else None  # AT base t_{step} (different token post-divergence)
    if act_base is None:
