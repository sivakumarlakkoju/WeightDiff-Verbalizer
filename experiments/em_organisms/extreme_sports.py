# %% [markdown]
# Activation diff NLA test — EXTREME-SPORTS
#
...
#   Early steps likely share the same tokens ("BASE jumping requires..."),
...
# %% [6/6] Finetuned-only generation verbalizations at extreme-sports-relevant tokens
# No base comparison — just: what does NLA say about finetuned's own activation
# at the steps where it writes the dangerous encouragement?

TELLTALE_KEYWORDS = ["without", "skip", "ignore", "no", "just", "immediately", "anyone", "training", "instructor", "safety", "gear", "equipment", "risk", "dangerous", "jump", "parachute", "rope", "harness", "helmet", "experience", "beginner"]
ft_target_steps = [
...
# %% [7/7] Safety-prompted NLA + activation diff at extreme-sports-relevant tokens
#
# Part D: same extreme-sports steps as Part C, but with a safety-aware template that
...
#         Note: models may diverge early, so diff includes both the LoRA
...
# ── Part D: safety-prompted NLA on finetuned extreme-sports tokens ─────────────
...
# ── Part E: activation diff at extreme-sports steps (ft − base, standard NLA) ──
lines.append("\n\n### EXTREME-SPORTS — Part E — Activation diff (ft − base) at extreme-sports steps (standard NLA)")
lines.append("Note: models may diverge early; diff at later steps includes content shift.")
